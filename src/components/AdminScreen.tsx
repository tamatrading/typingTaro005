...
const AdminScreen: React.FC<AdminScreenProps> = ({
  ...
  const clearAll = () => {
    setSelectedGroups(['basic']); // Always keep F/J practice
  };
      ...
      <div className="w-full max-w-2xl bg-white rounded-lg shadow-xl p-8">
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            >
              <Shuffle className={`w-5 h-5 ${isRandomMode ? 'text-purple-500' : 'text-gray-500'}`} />
              <span>ランダム出題</span>
              {isRandomMode && (
          ...
          <div className="mt-8 p-4 bg-gray-50 rounded-lg">
            ...
            <p className="mb-1">出題モード: {isRandomMode ? 'ランダム' : '順番通り'}</p>
            <p>落下速度: {speed}</p>
          </div>
        </div>
      </div>
