  ...
  const createNewWord = useCallback(() => {
    ...
    if (questionCount > 0 && questionCount % 4 === 3) {
      ...
      if (settings.isRandomMode) {
        // ランダムモードの場合、選択された文字からランダムに選択（F/Jは除外）
        const nonFJChars = availableCharacters.filter(
          (char) => char !== 'F' && char !== 'J'
        );
        text = nonFJChars[Math.floor(Math.random() * nonFJChars.length)];
      } else {
        // 通常モード：現在のステージの文字から選択
        const currentSet = stageSets[stage as keyof typeof stageSets];
        const nonFJChars = currentSet.filter(
          (char) => char !== 'F' && char !== 'J'
        );
        text = nonFJChars[Math.floor(Math.random() * nonFJChars.length)];
      }
      ...
      <div className="w-full max-w-2xl p-8 bg-gradient-to-b from-blue-100 to-blue-200 shadow-xl rounded-lg transition-transform duration-300 hover:scale-[1.02]">
        <div className="text-center mb-4">
          <div className="flex justify-between items-center mb-2">
            <h1 className="text-2xl font-bold">タイピング太郎 v005</h1>
            <button
